...
export class FunctionSection extends BaseSection {
  ...
  private readSignatures() {
    ...
    const [numberOfFunctions, lengthBytes] = leb.decodeULEB128(this.buf, pos);
    pos += lengthBytes;

    for (let i = 0; i < numberOfFunctions; i++) {
      const [typeIndex, typeIndexLength] = leb.decodeULEB128(this.buf, pos);
      pos += typeIndexLength;
  ...
  export(): Buffer[] {
    const output = [
      ...
      Buffer.from([]),
      Buffer.from(leb.encodeULEB128(this.signatures.length)),
      this.buf,
      ...this.newChunks,
    ];
